...
const deleteFromCloudinary = async (url: string) => {
  ...
  try {
    ...
    // cloudinary.uploader.destroy(publicId) by default assumes the file is an image.
    // But your videoFile is a video, and Cloudinary will ignore it unless you pass resource_type: "video".
    ...
    return NextResponse.json(
      { success: false, message: "Unable to delete file from the cloudinary" },
      { status: 500 }
    );
  }
};
